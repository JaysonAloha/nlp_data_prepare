...
"""导入位置错误"""
try:
    from scripts.tools.web_search import google_search_sync, SearchOptions
except ImportError:
...
def get_stock_news(ticker, max_news: int = 10, date: str = None) -> list:
    ...
    """补充功能，如果google数量不足，使用akshare补全"""
    if len(new_news_list)<max_news:
        print(f"Google获取新闻数量不足，使用akshare补充至{max_news}条")
        news_need=fetch_count-len(new_news_list)
        supplement_news_list = get_stock_news_via_akshare(ticker, news_need)
        new_news_list.extend(supplement_news_list)
